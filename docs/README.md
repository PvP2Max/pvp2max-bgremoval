...
WITHOUTBG_API_URL=http://withoutbg/api/remove-background # your self-hosted withoutbg endpoint (container port 80)
...
3) Access the app at `http://localhost:3000` (or via your tunnel). The app will call the bundled `withoutbg` service at `http://withoutbg/api/remove-background`.
